...
// Variable to store the server's IP and port
let serverIpPort = '';

// Function to fetch the server's IP and port
function fetchServerIpPort() {
    fetch('http://localhost:8080/get_ip')
        .then(response => response.json())
        .then(data => {
            serverIpPort = data.ip_port;
            console.log('Server IP and Port:', serverIpPort);
        })
        .catch(error => {
            console.error('Error fetching server IP and port:', error);
        });
}

// Call fetchServerIpPort initially to get the server's IP and port
fetchServerIpPort();
...
function animate() {
    if (canMakeRequest) {
        ...
        axios.get("http://" + str(serverIpPort) + "/get_data") // Make a GET request to fetch data from a local server
